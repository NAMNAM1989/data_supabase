  ...
  async function handleCreate(formData: FormData) {
    await runLocked(async () => {
      ...
      const result = await createCommodityAction({
        ...
        cargo_type: (formString(formData, "cargo_type") as any) || "GENERAL",
        ...
        default_packaging: (formString(formData, "default_packaging") as any) || "CARTON",
        status: "ACTIVE",
  ...
  async function handleUpdate(formData: FormData) {
    ...
    await runLocked(async () => {
      ...
      const result = await updateCommodityAction(editRow.id, {
        ...
        is_liquid: Boolean(formData.get("is_liquid")),
        notes: formString(formData, "notes"),
        status: (formString(formData, "status") as any) || editRow.status,
      });
      if (result.error) {
        toast.error(result.error);
        return;
